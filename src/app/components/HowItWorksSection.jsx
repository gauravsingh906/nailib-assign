...
export default function HowItWorksSection() {
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative font-[IntegralCF] py-32 px-4 sm:px-6 lg:px-8 overflow-hidden bg-gray-50"
      style={{
        perspective: '1000px'
      }}
    >
      ...
      >
        ...
        <div className="flex justify-center mb-8">
          <button 
            ...
            className={`px-4 py-2 rounded-full flex items-center text-sm font-medium transition-all duration-300 ${isRotated ? 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white' : 'bg-white text-gray-700 border border-blue-200 shadow-md'}`}
          >
        ...
        >
          ...
          <div className="relative h-full w-full">
            {steps.map((step, index) => {
              // Calculate position and style based on relation to active step
                      ...
                      <div className="space-y-3 mb-8">
                        {step.features.map((feature, i) => (
                          <div key={i} className="flex items-center">
                            ...
                            <span className={`${step.textColor} font-medium`}>{feature}</span>
                          </div>
                        ))}
                      </div>
                      
                      <button className={`self-start px-8 py-3 rounded-xl bg-gradient-to-r ${step.color} text-white font-bold transform transition-all duration-300 hover:scale-105 hover:shadow-lg`}>
                        Get Started
                      </button>
                    </div>
                            ...
                            {[
                              ...
                              >
                                <div className={`w-10 h-10 rounded-lg ${subject.color} mb-2 flex items-center justify-center`}>
                                  <span className="text-white font-bold">{subject.name.charAt(0)}</span>
                                </div>
                          ...
                          <div className="w-80 space-y-4">
                            <div className="flex items-center justify-between mb-2">
                              <div className="h-8 w-40 bg-gradient-to-r from-amber-500 to-orange-500 rounded-md flex items-center justify-center">
                                <span className="text-white text-sm font-bold">Your Progress</span>
                              </div>
                              <div className="text-amber-600 text-xl font-bold">78%</div>
                            </div>
                              ...
                              {[75, 90, 65].map((score, i) => (
                                <div key={i} className="rounded-xl bg-white p-3 border border-blue-100 shadow-md">
                                  <div className="text-center">
                                    <div className={`text-2xl font-bold ${score > 80 ? 'text-green-500' : score > 70 ? 'text-amber-500' : 'text-red-500'}`}>
                                      {score}%
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">Topic {i+1}</div>
                                  </div>
                                </div>
                              ))}
                            </div>
        ...
        {/* Call to Action */}
        <div className="mt-20 font-[IntegralCF] text-center">
          <div className="inline-block bg-gradient-to-r from-blue-500 via-indigo-500 to-cyan-500 p-px rounded-2xl">
            <button className="px-8 font-[IntegralCF] py-4 bg-white rounded-2xl text-blue-600 font-bold text-lg hover:bg-gray-50 transition-all duration-300 transform hover:scale-105 shadow-lg">
